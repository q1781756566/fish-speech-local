...
def init_model(checkpoint_path, device, precision, compile=False):
    ...
    from fish_speech.models.text2semantic.llama import BaseModelArgs, precompute_freqs_cis, _remap_fish_qwen3_omni_keys

    path_obj = Path(checkpoint_path)

    # 1. Load config + tokenizer
    config = BaseModelArgs.from_pretrained(str(path_obj))
    tokenizer = FishTokenizer.from_pretrained(path_obj)
    ...
    # 5. Rebuild non-persistent buffers (were meta, not in state_dict)
    #    Will be recreated at reduced max_seq_len in main(), use placeholder here
    model.register_buffer(
        "freqs_cis", torch.empty(0, device=device), persistent=False
    )
    model.register_buffer(
        "causal_mask", torch.empty(0, dtype=torch.bool, device=device), persistent=False
    )
    model.register_buffer(
        "fast_freqs_cis",
    ...
    logger.info(f"Restored model from checkpoint (meta device init)")
...
def main(
    ...
    # Rebuild buffers (were placeholder from meta device init)
    ...
    logger.info(f"Time to load model: {time.time() - t0:.02f} seconds (max_seq_len={max_seq})")
